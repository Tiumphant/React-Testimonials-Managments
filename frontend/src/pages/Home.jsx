...
export default function Home() {
  ...
  const limit = 3; // 3 testimonials per page
    ...
    const loadTestimonials = async () => {
      try {
        ...
        const res = await fetch(
          `http://localhost:5000/api/testimonials/approved?page=${page}&limit=${limit}`
        );
  ...
  useEffect(() => {
    const loadSummary = async () => {
      try {
        const res = await fetch(
          "http://localhost:5000/api/testimonials/summary"
        );
        const data = await res.json();
        ...
        {Array.from({ length: totalPages }, (_, i) => i + 1).map((num) => (
          <button
            ...
            className={`px-4 py-2 rounded-lg font-medium transition 
              ${num === page ? "bg-blue-500 text-white shadow-lg scale-105" : "bg-gray-200 hover:bg-gray-300"}`}
          >
            {num}
          </button>
        ))}
